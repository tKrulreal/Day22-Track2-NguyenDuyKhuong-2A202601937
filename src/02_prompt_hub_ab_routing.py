...
PROMPT_V1_NAME = "nguyen-duy-khuong-rag-v1"
PROMPT_V2_NAME = "nguyen-duy-khuong-rag-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
SYSTEM_V1 = """Bạn là trợ lý AI thân thiện.
Trả lời ngắn gọn (2-4 câu) dựa trên context được cung cấp.
Nếu không có thông tin trong context, hãy nói thẳng là không biết.
...
SYSTEM_V2 = """Bạn là chuyên gia phân tích AI.
Khi trả lời, hãy thực hiện các bước sau:
1) Đọc kỹ context và xác định các facts liên quan đến câu hỏi
2) Tóm tắt câu trả lời chính một cách rõ ràng
3) Trích dẫn nguồn từ context (nếu có)
4) Nêu rõ mức độ chắc chắn của câu trả lời (cao/trung bình/thấp)
Luôn dựa trên dữ liệu được cung cấp trong context, không suy đoán thêm."""
...
def push_prompts_to_hub(client: Client):
    ...
    """
    try:
        url = client.push_prompt(PROMPT_V1_NAME, object=PROMPT_V1, description="V1 - Phong cach ngan gon")
        print(f"✅ Đã push V1 → {url}")
    except Exception as e:
        print(f"⚠️  V1 lỗi: {e}")

    try:
        url = client.push_prompt(PROMPT_V2_NAME, object=PROMPT_V2, description="V2 - Phong cach co cau truc")
